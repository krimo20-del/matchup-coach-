// Prepare a bot-lane (ADC) bespoke-content authoring job for a champ.
// Bot has a verified favour-timeline store (the FIX object inside champ-data/_label-text-fixes.js,
// keyed by dataKey e.g. "aphelios_bot"), extracted into _label-text-fix-store.json. That store is
...
// Usage: node _bespoke_prep_bot.js <champSlug> <champDisp> <scratch>
const fs = require('fs'), path = require('path');
const champ = process.argv[2], champDisp = process.argv[3], scratch = process.argv[4];
const dataKey = champ + '_bot';
const wr = JSON.parse(fs.readFileSync(path.join(__dirname, '_wr_bot_' + champ + '.json'), 'utf8'));
const STORE = JSON.parse(fs.readFileSync(path.join(__dirname, '_label-text-fix-store.json'), 'utf8'));
const store = STORE[dataKey] || {};
const P = JSON.parse(fs.readFileSync(path.join(__dirname, '_bespoke_pool_bot.json'), 'utf8'));
const disp = P.disp;
global.window = {};
try { require(path.join(__dirname, 'champ-data', 'enemy-kits-bot.js')); } catch (e) {}
