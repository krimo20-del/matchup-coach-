// Assemble a bot-lane (ADC) bespoke content file from authored per-opponent JSON.
// Usage: node _bespoke_assemble_bot.js <champSlug> <scratchBespokeBotDir>
// Writes champ-data/content/bot/<champ>.js (a = '<champ>_bot', matches CHAMP_FULL key).
const fs = require('fs'), path = require('path');
const champ = process.argv[2];
const base = process.argv[3]; // .../scratchpad/bespoke-bot/<champ>
const job = JSON.parse(fs.readFileSync(path.join(base, 'job.json'), 'utf8'));
const outDir = path.join(base, 'out');
const champDisp = job.champDisp;
const dataKey = job.champDataKey; // '<champ>_bot'
...
const banner = `// champ-data/content/bot/${champ}.js — bespoke ${champDisp} BOT-lane (ADC) matchup data.
// WR = lolalytics Emerald+ headline matchup win rate (largest-sample row, lane=bottom).
// a = '${dataKey}' (matches the bot-lane CHAMP_FULL key so it never collides with any
...
let out = banner +
  ...
  '\n); }\n';
const outFileDir = path.join(__dirname, 'champ-data', 'content', 'bot');
fs.mkdirSync(outFileDir, { recursive: true });
fs.writeFileSync(path.join(outFileDir, champ + '.js'), out);
global.window = {};
require(path.join(outFileDir, champ + '.js'));
console.log('WROTE champ-data/content/bot/' + champ + '.js (' + out.length + ' bytes, ' + entries.length + ' entries, parses OK)');
