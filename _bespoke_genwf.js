// Generate the authoring workflow script for a champ from its job.json + groups.json.
// win[] is store-locked (authoritative chart timeline), so agents are told to copy it
// EXACTLY and make every why/prose/spike agree with each cell's colour in one pass.
// Usage: node _bespoke_genwf.js <champ> <scratch> <scriptsDir>
...
const script = `export const meta = {
  name: 'bespoke-${champ}',
  description: 'Author bespoke Reddit-informed top-lane matchup content for ${champDisp} (${job.matchups.length} matchups, chart-locked)',
  phases: [{ title: 'Author', detail: '${groups.length} agents, win[] locked to the verified favour timeline' }],
}
...
const results = await parallel(GROUPS.map((grp, gi) => () =>
  agent(
\`You are a Challenger-level LoL coach writing bespoke TOP-LANE matchup content for ${champDisp} ("you" = ${champDisp}). ${kitLine}

STEP 1 — Read the job file with Read: \${JOB}
It has { matchups:[ {opp, oppDisp, wr, games, lockWin} ] }. wr = ${champDisp}'s emerald+ win rate vs that opponent. lockWin is the AUTHORITATIVE 7-phase favour timeline (absolute champion display names) that colours the level chart — you MUST copy it verbatim into win[]; never change it.

STEP 2 — For EACH of these opponents: \${grp.join(', ')}
find its matchup and author a content object with EXACTLY this shape (house voice: concrete, mechanical, second-person, names real abilities, ~4-6 sentences per prose field):
{
 "a":"${champ}","b":"<opp slug>",
...
CRITICAL: every why, every spike, and the early/mid/late prose must be consistent with win[]'s colours and with the wr. Ground all claims in the real kits + the opponent's key cooldown.

STEP 3 — Write each opponent's object to \${OUT}/<opp slug>.json (compact JSON). Do all \${grp.length}.
Reply only: ${champ} g\${gi + 1}: <n> written.\`,
    { label: \`${champ}:g\${gi + 1}\`, phase: 'Author' }
  )
))
return { groups: GROUPS.length, done: results.filter(Boolean).length }
`;
const outPath = path.join(scriptsDir, 'bespoke-' + champ + '.js');
