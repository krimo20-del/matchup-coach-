// Assemble a mid-lane bespoke content file from authored per-opponent JSON.
// Usage: node _bespoke_assemble_mid.js <champSlug> <scratchBespokeMidDir>
// Writes champ-data/content/mid/<champ>.js (a = '<champ>_mid', matches CHAMP_FULL key).
const fs = require('fs'), path = require('path');
const champ = process.argv[2];
const base = process.argv[3]; // .../scratchpad/bespoke-mid/<champ>
const job = JSON.parse(fs.readFileSync(path.join(base, 'job.json'), 'utf8'));
const outDir = path.join(base, 'out');
const champDisp = job.champDisp;
const dataKey = job.champDataKey; // '<champ>_mid'
...
for (const m of job.matchups) {
  ...
  if (m.lockWin) e.win = m.lockWin; // enforce chart-lock
...
// Chart<->content risk report (chart = job.matchups[].lockWin, the generated mid chart).
const risk = [];
for (const m of job.matchups) {
...
if (risk.length) console.log('RISK (agent win != chart, whys need a look): ' + JSON.stringify(risk));
else console.log('RISK: none — every authored win[] matched the chart timeline.');
...
const banner = `// champ-data/content/mid/${champ}.js — bespoke ${champDisp} MID-lane matchup data.
// WR = lolalytics Emerald+ headline matchup win rate (largest-sample row, lane=middle).
// a = '${dataKey}' (matches the mid-lane CHAMP_FULL key so it never collides with any
// top-lane content for a champ of the same name). Pushed to MC_WR_TABLES / MC_CONTENT_EXTRA;
...
let out = banner +
  ...
  '\n); }\n';
const outFileDir = path.join(__dirname, 'champ-data', 'content', 'mid');
fs.mkdirSync(outFileDir, { recursive: true });
fs.writeFileSync(path.join(outFileDir, champ + '.js'), out);
global.window = {};
require(path.join(outFileDir, champ + '.js'));
console.log('WROTE champ-data/content/mid/' + champ + '.js (' + out.length + ' bytes, ' + entries.length + ' entries, parses OK)');
